...
import ClassEnrollmentManager from "@/components/admin/class-enrollment-manager";
...
export default function CatalogManager({
  ...
  const [managingClass, setManagingClass] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  function openCreate() {
    ...
    setFormStatus(
      statuses[0] || (resource === "programs" ? "draft" : "scheduled"),
    );
  ...
  async function deleteProgram(item: RecordItem) {
    if (resource !== "programs") {
      return;
    }

    const programName = String(item.data.title || "this program");
    ...
    try {
      const response = await fetch(`/api/admin/catalog/programs/${item.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || "Unable to delete program.");
      }
    ...
    <main className="p-5 sm:p-7 lg:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <SummaryCard label="Total" value={counts.total || 0} />

          <SummaryCard
            label={firstStatus ? pretty(firstStatus) : "Status"}
            value={firstStatus ? counts[firstStatus] || 0 : 0}
          />

          <SummaryCard
            label={secondStatus ? pretty(secondStatus) : "Status"}
            value={secondStatus ? counts[secondStatus] || 0 : 0}
          />

          <SummaryCard
            label={thirdStatus ? pretty(thirdStatus) : "Status"}
            value={thirdStatus ? counts[thirdStatus] || 0 : 0}
          />
        </section>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setManagingClass({
                                  ...
                                  name: String(item.data.name || "Class"),
                                })
                              }
                              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-[#8f0024]/20 bg-[#fff1f4] px-3 text-[9px] font-black text-[#8f0024] hover:bg-[#ffe5eb]"
                            >
                          ...
                          {resource === "programs" && (
                            <button
                              type="button"
                              onClick={() => deleteProgram(item)}
                              disabled={deletingId === item.id}
                              className="inline-flex h-8 items-center gap-1 rounded-md border border-red-200 px-3 text-[9px] font-black text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              {deletingId === item.id ? (
              ...
              {fields.map((field) => (
                ...
                >
                  <label className="mb-2 block text-[11px] font-black text-slate-700">
                    ...
                    {field.required && (
                      <span className="ml-1 text-[#8f0024]">*</span>
                    )}
                  </label>
      ...
      {resource === "classes" && managingClass && (
...
function SummaryCard({ label, value }: { label: string; value: number }) {
  return (
    <article className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-3 text-3xl font-black text-[#271a1e]">{value}</p>
    </article>
  );
}
