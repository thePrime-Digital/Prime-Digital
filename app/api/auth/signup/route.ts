...
export async function POST(
  ...
  if (!isPublicSignupRole(requestedRole)) {
    return NextResponse.json(
      { error: "Invalid signup role. Public signup is available only for students, faculty and clients." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (selfIamConfig) {
      try {
        ...
        if (status !== 409) {
          console.error("self-IAM signup error:", err);
          return NextResponse.json(
    ...
    const status: UserStatus = requestedRole === "faculty" ? "pending" : "active";
