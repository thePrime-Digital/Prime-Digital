...
export const SESSION_COOKIE_NAME = "prime_digital_session";
...
export async function createSessionToken(session: SessionData): Promise<string> {
  ...
  return new SignJWT({
    ...
    .setExpirationTime(currentTimestamp + SESSION_DURATION_SECONDS)
...
export async function verifySessionToken(token: string): Promise<SessionData> {
  ...
  if (!payload.sub) {
    throw new Error("Session token does not contain a user ID.");
  }

  if (typeof payload.email !== "string") {
    throw new Error("Session token does not contain an email.");
  }

  if (!isUserRole(payload.role)) {
    throw new Error("Session token contains an invalid role.");
  }
